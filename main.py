...
PRN_NUMBER = 2
CAL_TIME = [17, 45]  # hour, min

GROUND_TRUE = [14216.904936, 8447.065446, -20529.441834]
...
class GpsSat:
    ...
    def calc_tk(self):  # step 4 time 2015/2/1 (일요일) 17:45 -> GPS Week Second
        t = CAL_TIME[0] * 3600 + CAL_TIME[1] * 60
        tk = t - self.toe
        return tk
...
target_sat = PRN_list[f"{PRN_NUMBER}"]
GpsSat_list = []
for i in target_sat:
    GpsSat_list.append(GpsSat(i))

# 현재 시간에 맞는 가장 최적의 데이터값을 구한다.
best_data = None
